...
void main()
{
    ...
    vec3 v3Ray = v3Pos - u_camera_v3Position;
    float fFar = length(v3Ray);
    v3Ray /= fFar;

    vec3 v3Start = u_camera_v3Position;
    float fDepth = exp((u_atm_fPlanetRadius - u_camera_fHeight) / u_atm_fScaleDepth);
    float fCameraAngle = dot(-v3Ray, v3Pos) / length(v3Pos);
    float fLightAngle = dot(u_light_v3Direction, v3Pos) / length(v3Pos);
